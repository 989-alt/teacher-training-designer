...
> **주최사 지정 양식이 있는 경우(지식샘터 등) — 워크플로우 시작 전 먼저 판단한다.** 사용자가 "지식샘터 / 지식샘 / 직무연수 교재 / 주최사 양식 / 이 양식으로" 등을 언급하거나 특정 주최사에 제출하는 교재가 목적임이 드러나면, 산출물이 마크다운 설계안이 아니라 **주최사 지정 양식을 따르는(필수 슬라이드를 포함한) 16:9 PPTX 교재 덱**일 수 있다. 이 스킬은 특정 주최사의 양식을 내장하지 않으므로, 먼저 이렇게 확인한다:
>
> *"지식샘터(또는 해당 주최사) 제출용인가요? 지정된 PPT 양식·작성 가이드 파일을 올려주시면, 그 필수 슬라이드 구조(필수 섹션·색코딩·기재 항목 등)를 그대로 반영해 제작하겠습니다."*
>
> 사용자가 양식을 제공하면 `references/reference-driven-design.md`의 절차로 그 양식을 시각 언어·구조로 분해(토큰화)해 **필수 슬라이드를 빠짐없이 포함한** 덱을 만든다. 양식을 제공받기 전에는 필수 섹션을 임의로 창작하지 않는다. 역량 확인·대화형 확정 원칙(아래 Step 1~3)은 그대로 쓴다.
